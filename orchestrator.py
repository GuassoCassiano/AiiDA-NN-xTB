...
from aiida.orm import Str, Int, load_code, Group
...
def gather_results(*results):
    # This filters out any nodes that failed and returned None
    successful = [res for res in results if res is not None]
    print(f"Batch complete! {len(successful)} out of {len(results)} succeeded.")
...
def launch_batch(group_name, smiles_list, cluster_code_string, num_machines, num_mpiprocs_per_machine, wallclock):
    ...
    for i, smiles_string in enumerate(smiles_list):
        ...
        smiles_task = wg.tasks.add(smiles2structure, name=smiles_task_name)
        smiles_task.set({"smiles_node": Str(smiles_string)})
        
        # add the xTB CalcJob task
        xtb_task = wg.tasks.add(NNxTBCalculation, name=xtb_task_name)
        
        wg.links.add(smiles_task.outputs["result"], xtb_task.inputs["structure"])
        
        # feed the hardware settings to the CalcJob
        xtb_task.set({
            "code": cluster_code,
            "metadata": {
                "options": {
        ...
        final_outputs.append(xtb_task.outputs["results"])
        
    # add the gather node at the very end of the loop
    gather_node = wg.tasks.add(gather_results, name="final_tally")
    
    # wire all parallel outputs directly into the single gather node
    wg.links.add(final_outputs, gather_node.inputs["results"])
